...
function events_list_update() {
  events_list.length = 0;
  events_list.add(new Option('Update', 'update', false, event_number === null));
  let last = -1;
  for (let evnt of HTTPRequest('GET', '/cgi-bin/get-event.cgi').split('\n')) {
    if (evnt != '') {
      events_list.add(new Option(evnt, evnt, false, event_number == evnt));
      if (evnt > last) last = evnt;
    };
  };
  if (last >= 0 && event_number === null) load_event(last);
};

events_list_update();

function events_list_change() {
  if (events_list.value == 'update') {
    events_list_update();
    event_number = null;
    return;
  };

  load_event(events_list.value);
};

function load_event(evnt) {
  event_number = evnt;
  d3.csv('/cgi-bin/get-event.cgi?event=' + evnt, function(err2, rows2){
